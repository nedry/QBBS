      ...
      dirs.reverse.each {|dir|
        if(FileTest.directory?(dir))
         ...
         #   begin
              ...
              puts "-BOT: loading plugin #{tmpfilename}"
        ...
        #    rescue Exception => err
            ...
            #  puts "plugin #{tmpfilename} load failed\n" #+ err.inspect
...
puts "DEBUG: in delegate"
      @@plugins.values.uniq.each {|p|
      #  p.send method, *args
        if(p.respond_to? method)
          puts "DEBUG: We've got a winner"
          begin
            p.send method, *args
          rescue Exception => err
            #rescue TimeoutError, StandardError, NameError, SyntaxError => err
    ...
    def privmsg(m)
      ...
      if (@@plugins.has_key?(m.plugin) &&
        ...
        # @@bot.auth.allow?(m.plugin, m.source, m.replyto)
          ...
          puts "DEBUG: sending message to plugin #{m.plugin} #{@@plugins[m.plugin].privmsg(m)}"
        begin
