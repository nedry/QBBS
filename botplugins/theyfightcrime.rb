PlugMan.define :theyfightcrime do
  author "Mark Firestone"
  version "1.0.0"
  extends({ :main => [:bots] })
  requires []
  extension_points []
  params({ :description => "theyfightcrime => generate a random movie / TV scenario.", :cmd => "theyfightcrime"})
...
@SHE = [
    ...
    def do(m, options={})
        ...
        return ["Summary: He's #{he_things.join " "} She's #{she_things.join " "} They fight crime.",dest(m)]
    end
end
